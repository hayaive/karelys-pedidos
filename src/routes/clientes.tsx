...
import { Badge, Btn, Card, ConfirmDialog, Empty, Field, Input, Modal, Select } from "@/components/ui-kit";
...
export const Route = createFileRoute("/clientes")({
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Directorio de clientes con cédula, contacto y estado." },
    ],
  }),
  component: () => (
...
function Clientes() {
  ...
  const list = s.customers.filter(
    (c) =>
      ...
      (c.name.toLowerCase().includes(q.toLowerCase()) || c.cedula.toLowerCase().includes(q.toLowerCase())),
  );
  ...
  return (
    <>
      <PageHead
        title="Clientes"
        sub={`${s.customers.length} registrados`}
        action={
          can("edit_customers") && (
      ...
      <div className="mb-3 flex flex-col gap-2 sm:flex-row">
        <div className="relative flex-1">
          ...
          <Input className="pl-9" placeholder="Buscar por nombre o cédula" value={q} onChange={(e) => setQ(e.target.value)} />
        </div>
      ...
      <Card>
        {list.length === 0 ? (
          <Empty title="Sin clientes" sub="Crea el primer cliente para asociarlo a ventas y pedidos." />
        ) : (
          <div className="divide-y divide-border">
            {list.map((c) => (
              <div key={c.id} className="flex flex-col gap-2 px-4 py-3 sm:flex-row sm:flex-wrap sm:items-center sm:gap-3">
                <div className="flex min-w-0 flex-1 items-start gap-2">
                  ...
                  <div className="shrink-0">
                    <Badge tone={c.active ? "green" : "neutral"}>{c.active ? "Activo" : "Desactivado"}</Badge>
                  </div>
                </div>
                {can("edit_customers") && (
        ...
        {edit && (
          <div className="space-y-3">
            <Field label="Cédula" hint="Formato V-12345678">
              <Input value={edit.cedula ?? ""} onChange={(e) => setEdit({ ...edit, cedula: e.target.value })} />
            </Field>
            <Field label="Nombre">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Btn
                variant="amber"
                onClick={() => {
                  if (!validCedula(edit.cedula ?? "")) return toast.error("Cédula inválida (ej: V-12345678)");
